define([
  ...
  function(angular, $, kbn, _, config, moment, Modernizr) {
    ...
    module.service('dashboard', function($routeParams, $http, $rootScope,
      $injector, $location,
      ...
      var route = function() {
        ...
        if (!(_.isUndefined($routeParams.kbnType)) && !(_.isUndefined(
            $routeParams.kbnId))) {
          ...
          if (Modernizr.localstorage && !(_.isUndefined(window.localStorage[
              'dashboard'])) &&
        ...
        kbnIndex.collections(config.apollo_coll).then(function(p) {
          ...
          if (p.length > 0) {
            ...
            if (config.USE_ADMIN_CORES) {
              alertSrv.set('No collections',
                'There were no collections returned from Solr.',
                'info', 5000);
            }
          }
        });
      ...
      this.dash_load = function(dashboard) {
        ...
        if (dashboard.index.interval !== 'none' && filterSrv.idsByType(
            'time').length === 0) {
          self.refresh();
        }
      ...
      this.is_gist = function(string) {
        if (!_.isUndefined(string) && string !== '' && !_.isNull(string
            .match(gist_pattern))) {
      ...
      this.to_file = function() {
        var blob = new Blob([angular.toJson(self.current, true)], {
      ...
      this.set_default = function(dashboard) {
        if (Modernizr.localstorage) {
          window.localStorage['dashboard'] = angular.toJson(dashboard ||
            self.current);
      ...
      this.file_load = function(file) {
        return $http({
          ...
          alertSrv.set('Error', "Could not load <i>dashboards/" +
            file + "</i>. Please make sure it exists", 'error');
          return false;
        });
      };
      ...
      this.elasticsearch_load = function(type, id) {
        return $http({
          url: config.SYSTEM_BANANA_QUERY_PIPELINE +
            '/select?wt=json&q=banana_title_s:"' + id + '"',
          method: "GET",
          transformResponse: function(response) {
            response = angular.fromJson(response);
            var source_json = angular.fromJson(response.response.docs[
              0]['banana_dashboard_s']);

            if (DEBUG) {
              console.debug('dashboard: type=', type, ' id=', id,
                ' response=', response, ' source_json=',
                source_json);
            }
            ...
            return renderTemplate(JSON.stringify(source_json),
              $routeParams);
          }
        }).error(function(data, status) {
          if (status === 0) {
            alertSrv.set('Error', "Could not contact Solr at " +
              config.solr +
              ". Please ensure that Solr is reachable from your system.",
              'error');
          } else {
            alertSrv.set('Error',
              'Could not find dashboard named "' + id +
              '". Please ensure that the dashboard name is correct or exists in the system.',
              'error');
          }
          return false;
      ...
      this.script_load = function(file) {
        return $http({
          ...
          transformResponse: function(response) {
            /*jshint -W054 */
            var _f = new Function('ARGS', 'kbn', '_', 'moment',
              'window', 'document', 'angular', 'require',
              'define', '$', 'jQuery', response);
      ...
      this.elasticsearch_save = function(type, title, ttl) {
        ...
        if (type === 'dashboard') {
          id = save.title = _.isUndefined(title) ? self.current.title :
            title;
        }
      ...
      this.elasticsearch_list = function(query, count) {
        ...
        var request = sjs.Request().indices(config.banana_index).types(
          'dashboard');
      ...
      this.gist_list = function(id) {
        return $http.jsonp("https://api.github.com/gists/" + id +
