...
define([
  ...
  module.controller('subcollection', function($scope, $q, $timeout, timer,
    ...
    $scope.get_data = function() {
      ...
      _.each($scope.panel.queries.ids, function(id) {
        var temp_q = querySrv.getQuery(id) + fq + stats + wt_json +
          rows_limit;
        $scope.panel.queries.query += temp_q + '\n';
        // Set the additional custom query
        if ($scope.panel.queries.custom !== null) {
          request = request.setQuery(temp_q + $scope.panel.queries
            .custom);
        } else {
      ...
      $q.all(promises).then(function(results) {
        _.each(dashboard.current.services.query.ids, function(id,
          i) {
          ...
          if (!(_.isUndefined(results[i].error))) {
            $scope.panel.error = $scope.parse_error(results[i]
              .error);
            return;
          }
    ...
    $scope.populate_modal = function(request) {
      $scope.inspector = angular.toJson(JSON.parse(request.toString()),
        true);
    };
  });
      ...
      link: function(scope, elem) {
        ...
        elem.bind("plothover", function(event, pos, item) {
          if (item) {
            ...
            $tooltip
              .html(kbn.query_color_dot(item.series.color, 20) +
                ' ' + value.toFixed(0))
